...
> backends, plus a direct byte-for-byte CPU/CUDA parity check), but not yet
> independently reviewed or hardened for production. See
...
> tested, and packaged.  The CUDA backend is implemented, selected at build time, and has now been **built and run on real GPU hardware**
> (see [Verification status](#verification-status)) — it was previously implemented but had never actually been compiled.

![Anti-aliasing modes compared on the same scene, with a magnified crop of a slanted edge (top: full scene; bottom: zoomed edge)](docs/images/antialiasing_compare.png)

*Figure generated by the library's own `examples/antialiasing_compare.py` — real
rendered output from the verified CUDA build (copied from
`examples/output/` into `docs/images/` since example outputs are
regenerated/git-ignored scratch files), not a mock-up.*
...
20. [Disclaimer & Work-In-Progress](#disclaimer--work-in-progress)
...
This session built and ran both backends on real hardware (not just
compile-checked) for the first time:

| Path | Status |
|---|---|
| CPU build (`-DTRIANGLE_RASTERIZER_BACKEND=CPU`) | ✅ builds clean after fixing a real link error (see below) |
| C test suite (`triangle_rasterizer_test`), CPU | ✅ all checks pass |
| Python test suite (`tests/python/run_tests.py`), CPU | ✅ 31/32 pass, 1 correctly skipped (CUDA-only test) |
| CUDA build (`-DTRIANGLE_RASTERIZER_BACKEND=CUDA`, NVIDIA GB10 / sm_121, CUDA 13) | ✅ builds after fixing 4 real bugs (see below) — **the CUDA backend had apparently never actually been compiled before this pass** |
| C test suite, CUDA build | ✅ all checks pass, confirmed active backend = `"cuda"` (not the CPU fallback) |
| Python test suite, CUDA build | ✅ 32/32 pass, including the CUDA-only determinism check |
| Direct CPU vs CUDA parity check (200 random triangles, 256×256) | ✅ **byte-for-byte identical** output, confirming the documented CPU/CUDA parity guarantee |

Five real bugs were found and fixed during this verification pass:

1. **CPU/CUDA link failure**: `raster_internal.h` uses `lround()` from
   `<math.h>`, but `CMakeLists.txt` never linked `libm`. Fixed by adding
   `target_link_libraries(triangle_rasterizer PRIVATE m)` (non-MSVC only).
2. **CUDA build failure on newer toolkits**: `CMakeLists.txt`'s default
   `CMAKE_CUDA_ARCHITECTURES` included `sm_61`/`sm_70` (Pascal/Volta), which
   `nvcc` 13.x no longer supports as compile targets. Updated the default to
   `75;80;86;90;120` (Turing through Blackwell, including GB10/sm_121).
3. **`backend_cuda.cu` compile failure**: `cpu_fallback_rasterize` (the
   serial CPU fallback compiled into the CUDA translation unit) declared
   `unsigned char *restrict dst`, but `restrict` is a C99 keyword not valid
   in C++ (`.cu` files compile as C++). Fixed with the portable `__restrict__`
   spelling.
4. **`backend_cuda.cu` compile failure**: the host entry point declared
   `const BatchTri *d_tri`, then passed it as the (non-const) destination of
   `cudaMemcpy(..., cudaMemcpyHostToDevice)` and to `cudaFree()` — both
   require a non-`const` pointer. Fixed by dropping the stray `const`.
5. **`backend_cuda.cu` compile failure**: `goto done;` (used for the CUDA
   error-handling cleanup pattern) jumped forward over the declarations of
   `dim3 block` / `dim3 grid`, which C++ forbids (unlike C) when the target
   label is still in the same scope. Fixed by moving those two declarations
   (which only depend on `width`/`height`, known up front) above the first
   `goto`.

A sixth, non-CUDA bug was also found and fixed in the **Python test
infrastructure**: `tests/python/run_tests.py` (the dependency-free standalone
test runner) used `except Exception:` to catch `pytest.skip()`, but pytest's
`Skipped` exception subclasses `BaseException`, not `Exception` — so on any
machine with pytest installed, running the CPU-only build via this runner
crashed the entire suite instead of reporting one clean skip. Fixed by
explicitly catching `_pytest.outcomes.Skipped` and reporting it as `SKIP`.
...
(C unit tests, Python end-to-end tests, real CPU + CUDA GPU hardware,
byte-for-byte CPU/CUDA parity) but has **not** undergone independent
third-party review, fuzzing, or large-scale production use. Use at your own
risk; please open an issue if you find a bug.

### To-do / known limitations

- [ ] The CUDA backend was implemented but, prior to this verification pass,
      had never actually been built — CI (`.github/workflows/ci.yml`)
      currently only exercises the CPU backend; a CUDA-enabled CI runner is
      not yet set up.
- [ ] No fuzz-testing of malformed/adversarial input (extreme/NaN-adjacent
      vertex coordinates beyond the explicit non-finite check, degenerate
      geometry at scale, very large triangle counts).
...
- [ ] No large-scene (>100k triangle) stress/perf benchmark yet.
- [ ] No packaged releases (PyPI wheel, versioned GitHub Releases) yet —
      build from source only.
- [ ] Anti-aliasing and fixed-point precision paths are cross-checked against
      the float/AA_NONE path numerically, but not against an independent
      reference rasterizer (e.g. a well-known rendering library).
